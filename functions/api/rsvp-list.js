...
export async function onRequestGet({ request, env }) {
  ...
  if (url.searchParams.get('format') === 'csv') {
    const rows = [['姓名', '手机', '人数', '时间']].concat(
      items.map(i => [i.name, i.phone, i.count, i.ts])
    );
  ...
  const rows = items.map((i, idx) =>
    `<tr><td>${idx + 1}</td><td>${esc(i.name)}</td><td><a href="tel:${esc(i.phone)}">${esc(i.phone)}</a></td><td>${esc(i.count)}</td><td>${esc(new Date(i.ts).toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai' }))}</td></tr>`
  ).join('');
...
<table><thead><tr><th>#</th><th>姓名</th><th>手机</th><th>人数</th><th>提交时间</th></tr></thead>
<tbody>${rows || '<tr><td colspan="5" style="text-align:center;color:#999">暂无提交</td></tr>'}</tbody></table>
</body></html>`;
